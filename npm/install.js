...
function getPlatformInfo() {
  ...
  return {
    ...
    url: `https://github.com/Blankeos/cftun/releases/download/v${VERSION}/${BINARY_NAME}-${target}${extension}`,
  };
}
...
function logInstallFailure(error) {
  ...
  console.error("\nYou can install cftun directly using:");
  console.error(
    'curl --proto "=https" --tlsv1.2 -LsSf https://github.com/Blankeos/cftun/releases/latest/download/cftun-installer.sh | sh',
  );
}
...
async function install({ exitOnComplete = false } = {}) {
  ...
  console.log(`cftun v${VERSION} installed successfully!`);
